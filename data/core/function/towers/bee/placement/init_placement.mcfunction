...
$scoreboard players set @n[tag=tower-barrel-marker] game_money $(sell_cost)
execute as @n[tag=bee-center-marker,tag=defense.off] run tag @s remove defense.off
execute as @n[tag=tower-barrel-marker] run function core:towers/global/rotate_back
...
execute if block ~4.5 -60 ~ #core:path_blocks run summon minecraft:marker ~4.8 -57.5 ~ {Tags:["defense.bee_deployer"]}
execute as @e[tag=defense.bee_deployer] at @s if block ~ -62 ~ minecraft:cyan_wool run rotate @s 0 0
execute as @e[tag=defense.bee_deployer] at @s if block ~ -62 ~ minecraft:blue_wool run rotate @s -90 0
execute as @e[tag=defense.bee_deployer] at @s if block ~ -62 ~ minecraft:light_blue_wool run rotate @s -180 0
execute as @e[tag=defense.bee_deployer] at @s if block ~ -62 ~ minecraft:purple_wool run rotate @s 90 0
