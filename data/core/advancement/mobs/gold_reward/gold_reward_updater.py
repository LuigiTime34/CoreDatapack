...
def create_advancement_json(mob_name, base_path):
    ...
    try:
        ...
        advancement = {
            ...
            "rewards": {
                "function": f"core:monsters/gold_reward/{mob_lowercase}_gold"
            }
        }
...
def create_mcfunction_file(mob_name, gold_reward, base_path):
    ...
    try:
        ...
        mcfunction_content = f"""advancement revoke @s only core:mobs/gold_reward/{mob_lowercase}_reward

playsound minecraft:entity.experience_orb.pickup master @s ~ ~ ~ 1.0 1.0
scoreboard players remove $mobs_left defense.wave 1

scoreboard players add $money game_money {str(gold_reward).strip()}
scoreboard players set $recent_kill game_money 20
scoreboard players add $recent_money game_money {str(gold_reward).strip()}
