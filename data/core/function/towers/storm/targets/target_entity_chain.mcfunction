...
$execute as @n[tag=monster,tag=towers.target] at @s run function core:towers/storm/chain {"damage":"$(damage)","blow_power":"$(blow_power)","max_chain_length":"$(max_chain_length)","chain_damage":"$(chain_damage)","chain_radius":"$(chain_radius)"}