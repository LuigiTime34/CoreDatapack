...
$execute as @n[tag=monster,tag=towers.target] at @s run function core:towers/storm/blow_back_freeze {"damage":"$(damage)","blow_power":"$(blow_power)","freeze_power":"$(freeze_power)","freeze_time":"$(freeze_time)"}