...
$place template core:towers/panda/base ~$(x_offset) ~ ~$(z_offset) $(rotation)
...
execute as @n[tag=panda-center-marker,tag=defense.off] run tag @s remove defense.off
execute as @n[tag=tower-barrel-marker] run function core:towers/global/rotate_back
execute as @n[tag=defense.panda] run function core:towers/global/rotate_back
