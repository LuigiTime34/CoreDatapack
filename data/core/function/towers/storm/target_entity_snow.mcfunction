...
$execute as @n[tag=defense-monster,tag=defense-target] at @s run function core:towers/storm/blow_back_freeze {"damage":"$(damage)","blow_power":"$(blow_power)","freeze_power":"$(freeze_power)","freeze_time":"$(freeze_time)"}