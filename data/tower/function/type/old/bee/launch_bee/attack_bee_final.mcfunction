execute as @e[distance=..4.95,tag=towers.bee_deployer] at @s anchored eyes positioned ^ ^ ^ positioned ~ -58.5 ~ anchored feet run function tower:bee/raycast/start_ray
execute unless entity @n[tag=towers.found_target] run return fail

execute if score @s towers_bee_cooldown matches 5.. run return fail
...
$execute at @e[tag=towers.bee_deployer,distance=..4.95,tag=towers.found_target] run summon text_display ~ ~ ~ {Tags:["towers.bee_display"],teleport_duration:1,Passengers:[{id:"minecraft:bee",AngerTime:10000,NoAI:1b,Team:"defense_player",Tags:["towers.bee"],Health:$(damage),Invulnerable:1b,attributes:[{id:"attack_knockback",base:$(speed)},{id:"attack_damage",base:0}]}]}
