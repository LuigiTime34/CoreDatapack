...
execute as @e[tag=chair] at @s if block ~ ~ ~ minecraft:air run tp @s ~ ~-1 ~
...
execute as @a[gamemode=adventure,tag=!started] if score $menumusic music matches 1.. run scoreboard players remove $menumusic music 1
...
# Detect start
execute as @p[gamemode=adventure,tag=!escape-start] at @s if block ~1 ~ ~ minecraft:black_concrete run function core:escape/start
...
# Door open
execute unless block 0 -58 -23 minecraft:air as @p[gamemode=adventure,tag=!memory-started] positioned 0 -59 -22 if entity @s[distance=..1] run setblock 0 -55 -26 minecraft:redstone_block
