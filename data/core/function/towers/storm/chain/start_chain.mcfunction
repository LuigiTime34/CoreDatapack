...
# Summon storm cloud
summon minecraft:marker ~ -53 ~ {Tags:["defense.storm_cloud"]}
execute as @e[tag=defense.storm_cloud] at @s run particle minecraft:campfire_cosy_smoke ~ -52 ~ 0.7 0.4 0.7 0 300 force
# Initial raycast to the first target
execute as @n[tag=defense.storm_cloud] at @s anchored eyes run function core:towers/storm/chain/starter_raycast with storage core:defense.storm_raycast
execute as @n[tag=defense.first_chain_target] at @s run function core:towers/storm/chain/chain_loop with storage core:defense.storm_raycast
# Clean up
