execute as @e[distance=..4.95,tag=defense.bee_deployer] at @s anchored eyes positioned ^ ^ ^ positioned ~ -58.5 ~ anchored feet run function core:defense/towers/bee/raycast/start_ray
execute unless entity @n[tag=defense.found_target] run return fail

execute if score @s defense.bee_cooldown matches 5.. run return fail
