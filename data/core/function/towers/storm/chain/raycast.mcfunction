...
# Looping
$execute facing entity @n[tag=!defense.in_chain,tag=defense.chain_target] feet positioned ^$(x) ^$(y) ^.1 unless entity @n[tag=!defense.in_chain,tag=defense.chain_target,distance=..0.3] run return run function core:towers/storm/chain/raycast with storage core:storm_raycast
