$place template core:towers/bee/upgrade$(building) ~$(x_offset) ~ ~$(z_offset) $(rotation)
...
kill @s
kill @n[tag=towers.bee_center]
kill @n[tag=towers.barrel_display]
tag @n[tag=towers.bee_center,tag=towers.off] remove defense.off
