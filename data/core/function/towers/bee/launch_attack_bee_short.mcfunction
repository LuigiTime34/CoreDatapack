...
execute as @e[distance=..4.95,tag=defense.bee_deployer] at @s anchored eyes positioned ^ ^ ^ positioned ~ -58.9 ~ anchored feet run function core:towers/bee/raycast_short/start_ray
...
execute if entity @s[tag=upgrade_attack2] run scoreboard players add @s towers_bee_cooldown 1
$execute as @s[tag=upgrade_attack2] run execute if score @s towers_bee_cooldown matches 6.. run return run function core:towers/bee/launch_queen_bee {"damage":"$(damage)","speed":"$(speed)","cooldown":"$(cooldown)"}
