...
execute if score #hit vdvcasttemp matches 0 positioned ~-0.05 ~-0.05 ~-0.05 as @e[type=minecraft:interaction,dx=0,tag=defense.tower_placer,tag=!defense.has_tower,tag=!vdvray,sort=nearest] run function core:defense/towers/global/squares/raycast/check_hit_entity
