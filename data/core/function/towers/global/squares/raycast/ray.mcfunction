...
execute unless block ~ ~ ~ #core:blocks run function core:towers/global/squares/raycast/hit_block
scoreboard players add #distance vdvcasttemp 1

#Advance forward and run the ray again if no entity and/or block was found.

execute if score #hit vdvcasttemp matches 0 if score #distance vdvcasttemp matches ..100 positioned ^ ^ ^0.1 run function core:towers/global/squares/raycast/ray