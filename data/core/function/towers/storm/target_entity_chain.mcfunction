...
$execute as @n[tag=defense-monster,tag=defense-target] at @s run function core:towers/storm/blow_back_chain {"damage":"$(damage)","blow_power":"$(blow_power)","max_chain_length":"$(max_chain_length)","chain_damage":"$(chain_damage)","chain_radius":"$(chain_radius)"}