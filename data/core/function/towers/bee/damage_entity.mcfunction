execute on passengers if entity @s[type=minecraft:bee,tag=defense.bee] store result score @s game_money run attribute @s minecraft:attack_damage get
execute on passengers if entity @s[type=minecraft:bee,tag=defense.bee] if score @s game_money matches 1.. run function core:towers/bee/add_money with storage core:defense.bee_damage
