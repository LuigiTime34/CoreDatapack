...
execute as @e[distance=..4.95,tag=towers.bee_deployer] at @s anchored eyes positioned ^ ^ ^ positioned ~ -58.9 ~ anchored feet run function tower:bee/raycast_short/start_ray
...
$execute as @s[tag=towers.upgrade_attack2] run execute if score @s towers_bee_cooldown matches 6.. run return run function tower:bee/launch_bee/queen {"damage":"$(damage)","speed":"$(speed)","cooldown":"$(cooldown)"}
